...
\begin{document}
	...
	\label{sec:intro}
		...
		%
		Sensor platforms for real time monitoring of carbon dioxide (\ce{CO2}) and methane (\ce{CH4}) gas concentrations are of interest as these gases can impact animal health and crop growth.
		%
		...
		%
		To address this issue, an array of commercially available, low-cost sensors for \ce{CO2} and \ce{CH4} were evaluated at concentrations typical to those required for environmental monitoring, and monitoring around carbon sequestration operations and oil fields.
		%% PARA
		...
		%
		Optical methods of detecting \ce{CO2} and \ce{CH4} are based on measuring the absorption of light at \SI{2352}{\per\centi\meter} and \SI{3015}{\per\centi\meter}, respectively~\cite{frodl_high-precision_2006,zhu_one_2012}.
		%
		...
		%
		Use of NDIR detectors for \ce{CH4} is limited in practical applications due to its lower absorption coefficient and overlapping symmetric C-H stretches.
		%
		...
		Chemiresistive sensors for detection of \ce{CH4} typically use a thin oxide film~\cite{neri_first_2015}.
		%
		...
		%
		When the film is exposed to \ce{CH4}, the molecule adsorbs and reacts with surface oxygen species resulting in a change in the electrical conductivity~\cite{wang_metal_2010,prudenziati_thick-film_1986}.
		%
		Since the sensor depends on chemical reactions involving both atmospheric oxygen and \ce{CH4} for proper functioning, the measurement is dependent on the partial pressures of these gases.
		%
		...
		%
		However, similar to the optical sensors for \ce{CH4}, the selectivity was not explored but should be considered when integrating these sensors into a sensor platform.
		%% PARA
	...
	\section{Results and Discussion}
		...
		\label{sec:time}
			...
			Figure~\ref{fig:peakdetailco2} shows the response of two selected \ce{CO2} sensors, the K-30 and Gascard, to 600 ppm and 1400 ppm concentration steps of \ce{CO2}.
			%
			...
			%
			It was observed that all the optical sensors responded quickly to an increase in \ce{CO2} inside the environmental chamber, limited only by the internal sampling rate of the sensor.
			
			\begin{figure}[!t]
				\centering
				\includegraphics[width=\columnwidth]{honey3.pdf}
				\caption{The Gascard (top) and K-30 (bottom) sensor response over time at high (1433 ppm, plotted with ``$\times$'' on the plot) and low (577 ppm, plotted with ``$+$'') concentrations of \ce{CO2}.
				%
		...
		\subsection{Calibration Result}
			...
			\begin{figure}[!t]
				...
				\caption{The response of the Gascard and MQ-4 sensors with respect to the concentration reported from the California Analytical Instruments Inc. ZRE Non-Dispersive Infrared Analyzer for different concentrations of either \ce{CO2} or \ce{CH4}, respectively.
					%
					The top part of the figure shows the response of an optical \ce{CO2} sensor with the expected linear behavior.
					%
					The bottom part of the figure shows the response of a chemiresistive \ce{CH4} sensor (MQ-4, response is expressed in conductivity) with a fit to the non-linear response}
				\label{fig:gascardcodiff}
			\end{figure}
			
			The lower plot in Figure~\ref{fig:gascardcodiff} shows the response of a chemiresistive \ce{CH4} sensor (MQ-4, response is expressed in conductivity) with non-linear fit to a  Langmuirian form (Equation~\ref{eq:langmuirian}).
			%
		...
		\subsection{Precision and Baseline Noise Tests}
			...
			%
			In most cases, the precision of the sensors was determined by a \SI{20}{\hour} to \SI{30}{\hour} data collection time at a known concentration (400 ppm for \ce{CO2} and 0 ppm for \ce{CH4} based on previously discussed atmospheric baselines) under uniform flow.
			%
			...
			\begin{figure}[!t]
				...
				\caption{Long term baseline data were collected at the atmospheric baseline conditions (using a bottle of compressed medical grade breathing air (approximately 0 ppm \ce{CH4}), in the gas mixing chamber.
				%
		...
		\subsection{Limits of Detection}	
			...
			The limit of detection for the Dynament single analyte \ce{CH4} sensor (MSH-P/HC) demonstrates the important influence of calibration error on the reported limit.
			%
	...
	\section{Conclusion}
		...
		The performance of a sensor platform for environmental or industrial monitoring is sensitive to the cost and performance of the individual sensor elements.
		%
		Thus, the detection limits, accuracy, and precision for a range of potential sensors for measuring ppm concentrations of \ce{CO2} and \ce{CH4} for low-cost monitoring instrumentation were quantified.
		%
		...
		For \ce{CO2}, the Gascard sensor had high performance based on the low errors reported in Table~\ref{tab:stddev}, but is comparatively more expensive than the other sensors investigated.
		%
		...
		For \ce{CH4}, there is no single sensor which meets the ppm or even sub-ppm sensitivity required for environmental monitoring.
		%
		The limit of detection for the selected relativity low-cost optical \ce{CH4} sensors reported in Table~\ref{tab:stddev}, collectively show that these NDIR sensors are not suitable for monitoring environmental levels of \ce{CH4} around the global average.
		%
		...
		%
		With an increasing focus on remote sensing technology, more precise low-cost \ce{CH4} sensors are required to produce accurate data at expected atmospheric concentrations.
		%
